...
<div id="searchheader">
    <div id="foundresults">
        <?php if(!isset($this->params['models']['Bookmark'])): ?>
            <?php //__('Objekte'); ?><?=count($this->Session->read('objectSlider'));?>
            ...
            <?php endif;?>
        <?php else: ?>
            <?php echo __('Ihre Merkliste enthält'); ?> <?=count($this->Session->read('objectSlider'));?> <?php if(count($this->Session->read('objectSlider')) == 1) echo __('Immobilie'); ?><?php if(count($this->Session->read('objectSlider')) != 1) echo __('Immobilien'); ?>
        <?php endif; ?>
    </div>
...
if($this->Session->read('Config.language') != DEFAULT_LANGUAGE){
    ...
    if (isset($this->params['catname'])) {
        ...
        if(isset($this->params['cat'])) $arg2['cat'] = $this->params['cat'];
    }
    ...
    if(!empty($this->params['named'])){
        if(isset($this->params['named']['c'])) $arg2['c'] = $this->params['named']['c'];
        if(isset($this->params['named']['r'])) $arg2['r'] = $this->params['named']['r'];
        if(isset($this->params['named']['p'])) $arg2['p'] = $this->params['named']['p'];
        if(isset($this->params['named']['ref'])) $arg2['ref'] = $this->params['named']['ref'];
        if(isset($this->params['named']['cat'])) $arg2['cat'] = $this->params['named']['cat'];
    }
...
<!--         </ul>-->
    ...
    <div id="pricesort"><?php echo __('Preis:'); ?>
        <?php
...
<?php if(isset($objects) && count($objects) > 0):?>
    ...
    <div class="searchresultrows" id="row_<?=$data['Gesamtobjekt']['OBJ_ID']?>"><a name="<?=$data['Gesamtobjekt']['OBJ_ID']?>" />
        ...
        <h4 class="orange">
            <?=$this->Html->link($data['Gesamtobjekt']['OBJ_UET_UEBERSCHRIFT'],
                $this->Flags->changeUrl(
                    $this->Session->read('Config.language'),
                        Inflector::slug($linkcity).'_'.
                        Inflector::slug($data['Gesamtobjekt']['OBI_UET_BESCHREIBUNG']).'-'.
        ...
        <div class="part2">
            ...
            <div class="info_rows"><?php echo __('Mallorca'); ?><?php if($data['Gesamtobjekt']['REG_ID'] > 1){?>, <?=$data['Gesamtobjekt']['REG_NAME']?><?php if($data['Gesamtobjekt']['BER_EXPOSERELEVANT'] == 1) echo ', '.$data['Gesamtobjekt']['BER_NAME'];  ?><span style="float:right"><img src="/img/dot.png" class="map_icon" id="OBJ<?=$data['Gesamtobjekt']['OBJ_ID']?>lati<?=$coords['reg_latitude'];?>long<?=$coords['reg_longitude'];?>" rel="#googlemap" /></span><?php } ?></div>
            <?php
                if(is_array($data['Gesamtobjekt']['EIAS'])):
                $rent = false;
                $onlyrent = false;
                foreach($data['Gesamtobjekt']['EIAS'] as $k => $eia): //debug($eia);
                    if($k == 'PLOT' && $eia != '-') echo '<div class="info_rows"><span class="spec_title">'.__('Grundstück').': </span>'.$eia.' m²</div>';
                    if($k == 'LIVING' && $eia != '-') echo '<div class="info_rows"><div class="spec_title">'.__('Wohnfläche').': </div>'.$eia.' m²</div>';
                    if($k == 'YEAR' && $eia != '-') echo '<div class="info_rows"><div class="spec_title">'.__('Baujahr').': </div>'.$eia.'</div>';
                    if($k == 'ROOM' && $eia != '-') echo '<div class="info_rows"><div class="spec_title">'.__('Schlafzimmer').': </div>'.$eia.'</div>';
                    if($k == 'BATH' && $eia != '-') echo '<div class="info_rows"><div class="spec_title">'.__('Bäder').': </div>'.$eia.'</div>';
            ...
            <div class="info_rows<?=($onlyrent)?'_rent':'';?>"><div class="spec_title"><?php echo __('Kaufpreis'); ?>:</div><?=$data['Gesamtobjekt']['OBJ_PREIS']?><?php if(!empty($data['Gesamtobjekt']['OBJ_UET_PREISZUSATZ']) && $data['Gesamtobjekt']['OBJ_PREIS'] != 0):?> | <?=$data['Gesamtobjekt']['OBJ_UET_PREISZUSATZ']?><?php endif; ?></div>

            <?php if($rent): ?>
                <?php if($onlyrent): ?>
                    <div class="info_rows"><div class="spec_title"><?php echo __('Mietpreis'); ?>:</div>&#8364; <?=$onlyrentprice?></div>
                <?php else: ?>
<!--                    <div class="info_rows--><?//=($rent)?'_rent':'';?><!--"><div class="spec_title">--><?php //__('Mietpreis'); ?><!--:</div>--><?php //__('Preis auf Anfrage'); ?><!--</div>-->
                    <div class="info_rows<?=($rent)?'_rent':'';?>"><?php echo __('Objekt kann angemietet werden, Preis auf Anfrage'); ?></div>
                <?php endif; ?>
            <?php endif; ?>
        ...
        <div class="part3">
            ...
            <div class="info_col"><?=$text?><span class="more_details">
                <?=$this->Html->link(__('mehr Details'),
                    $this->Flags->changeUrl(
                        $this->Session->read('Config.language'),
                            Inflector::slug($linkcity).'_'.
                            Inflector::slug($data['Gesamtobjekt']['OBI_UET_BESCHREIBUNG']).'-'.
