...
<meta name="description" content="<?php if(isset($metadesc)){ echo $metadesc; }else{ echo __('Immobilien auf Mallorca ✓ Finestproperties ist Ihr ★★★★★ Immobilienmakler auf Mallorca wenn es um exklusive Immobilien, Villas, Fincas und Apartments auf Mallorca geht'); }?>" />
<meta name="keywords"    content="<?php if(isset($metakeys)){ echo $metakeys; }else{ echo __('Immobilien Mallorca, Immobilienmakler Mallorca, Immobilien Mallorca kaufen, Exklusive Mallorca Immobilien, Apartments Mallorca, Fincas Mallorca, Villas Mallorca'); }?>" />
...
<?php
    ...
    if(isset($this->params) && $this->params['controller'] == 'gesamtobjekt' || $this->params['controller'] == 'content') echo $this->Html->css('nivo-slider');
...
<div class="header" id="mainHeader">
    <?=$this->Html->link(
        $this->Html->Image('Immobilien-Mallorca-Finest-Properties-Mallorca.png', array('class' => 'logo', 'alt' => __('Immobilienmakler Finest Properties'))),
            $this->Flags->changeUrl($this->Session->read('Config.language'), '/'),
            array('escape' => false)
    );?>
    <div class="top_nav">
        <ul>
...
<!--            <li>--><?//=$this->Html->link(__('Kontakt'), $this->Flags->changeUrl($this->Session->read('Config.language'), array('controller' => 'contacts', 'action' => 'show', 'multiple' => 1), true, Inflector::slug(__('URLKontakt'))));?><!--</li>-->
